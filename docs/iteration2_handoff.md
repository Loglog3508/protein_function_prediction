...
4. 若开展 GPU 路线，优先复用 `E:\CUDA\envs\protein-gpu`。当前 Hugging Face 缓存未发现已下载的蛋白预训练模型，使用外部权重前须确认竞赛规则。
5. 新模型产生验证分数后，再考虑按标签选择模型或 rank/概率校准融合；不要直接重复已经失败的全局简单加权。

## 复现命令

```powershell
E:\CUDA\envs\protein-gpu\python.exe -m pytest -q -p no:cacheprovider

E:\CUDA\envs\protein-gpu\python.exe -m src.train `
  --config configs\iteration1_kmer35_sgd_full.json --evaluate

E:\CUDA\envs\protein-gpu\python.exe -m src.thresholds `
  --scores artifacts\runs\EXP-20260923-025-iteration1-kmer35-sgd-full\scores.npz `
  --train data\train.csv `
  --output-prefix artifacts\metrics\EXP-20260923-025-iteration1-kmer35-sgd-thresholds `
  --seed 42 --holdout-fraction 0.5 --shrinkage 25
```

筛选器使用方式：

```powershell
E:\CUDA\envs\protein-gpu\python.exe -m src.sweep_sgd `
  --config configs\iteration1_vocab120k_screen.json
```
...
以下文件按 `.gitignore` 不进入 Git，但当前工作区已存在，可直接用于第二轮：
